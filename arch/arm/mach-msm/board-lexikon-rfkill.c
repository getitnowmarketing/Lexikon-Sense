...
/* Control bluetooth power for lexikon platform */
...
extern int lexikon_wifi_bt_sleep_clk_ctl(int on, int id);

/* bt initial configuration */
static uint32_t lexikon_bt_init_table[] = {

	/* BT_RTS */
	PCOM_GPIO_CFG(LEXIKON_GPIO_BT_UART1_RTS,
				0,
				GPIO_OUTPUT,
				GPIO_NO_PULL,
				GPIO_8MA),
	/* BT_CTS */
	PCOM_GPIO_CFG(LEXIKON_GPIO_BT_UART1_CTS,
				0,
				GPIO_INPUT,
				GPIO_PULL_UP,
				GPIO_8MA),
	/* BT_RX */
	PCOM_GPIO_CFG(LEXIKON_GPIO_BT_UART1_RX,
				0,
				GPIO_INPUT,
				GPIO_PULL_UP,
				GPIO_8MA),
	/* BT_TX */
	PCOM_GPIO_CFG(LEXIKON_GPIO_BT_UART1_TX,
				0,
				GPIO_OUTPUT,
				GPIO_NO_PULL,
				GPIO_8MA),
	...
	/* BT_RESET_N */
	PCOM_GPIO_CFG(LEXIKON_GPIO_BT_RESET_N,
				0,
				GPIO_OUTPUT,
				GPIO_NO_PULL,
				GPIO_4MA),
	/* BT_SHUTDOWN_N */
	PCOM_GPIO_CFG(LEXIKON_GPIO_BT_SHUTDOWN_N,
				0,
				GPIO_OUTPUT,
				GPIO_NO_PULL,
				GPIO_4MA),
	...
	/* BT_HOST_WAKE */
	PCOM_GPIO_CFG(LEXIKON_GPIO_BT_HOST_WAKE,
				0,
				GPIO_INPUT,
				GPIO_PULL_DOWN,
				GPIO_4MA),
	/* BT_CHIP_WAKE */
	PCOM_GPIO_CFG(LEXIKON_GPIO_BT_CHIP_WAKE,
				0,
				GPIO_OUTPUT,
				GPIO_NO_PULL,
				GPIO_4MA),
...
static void lexikon_config_bt_init(void)
{
	lexikon_wifi_bt_sleep_clk_ctl(CLK_ON, ID_BT);
	mdelay(2);

	/* set bt initial configuration*/
	config_bt_table(lexikon_bt_init_table,
				ARRAY_SIZE(lexikon_bt_init_table));
	mdelay(2);

	/* BT_RESET_N */
	gpio_configure(LEXIKON_GPIO_BT_RESET_N,
				GPIOF_DRIVE_OUTPUT | GPIOF_OUTPUT_LOW);
	mdelay(1);
	/* BT_SHUTDOWN_N */
	gpio_configure(LEXIKON_GPIO_BT_SHUTDOWN_N,
				GPIOF_DRIVE_OUTPUT | GPIOF_OUTPUT_LOW);
	mdelay(1);

	/* BT_CHIP_WAKE */
	gpio_configure(LEXIKON_GPIO_BT_CHIP_WAKE,
				GPIOF_DRIVE_OUTPUT | GPIOF_OUTPUT_LOW);

	/* BT_RTS */
	gpio_configure(LEXIKON_GPIO_BT_UART1_RTS,
	...
	/* BT_TX */
	gpio_configure(LEXIKON_GPIO_BT_UART1_TX,
				GPIOF_DRIVE_OUTPUT | GPIOF_OUTPUT_HIGH);
...
static void lexikon_config_bt_off(void)
{
	...
	lexikon_wifi_bt_sleep_clk_ctl(CLK_OFF, ID_BT);
	mdelay(2);
}
...
static int lexikon_rfkill_probe(struct platform_device *pdev)
{
	...
	lexikon_config_bt_init();	/* bt gpio initial config */
