 ...
 *
 ****************************************************************************/
#ifndef CONFIG_USB_ANDROID_QCT_DIAG
static void smd_diag_notify(void *priv, unsigned event)
#else
void bt_smd_diag_notify(void *priv, unsigned event)
#endif
{
	int bytesInSmdBuf;
...
#ifdef CONFIG_USB_ANDROID_QCT_DIAG
			diag_smd_enable(ctxt->ch, "btdiag_ioctl", SMD_FUNC_OPEN_BT);
#else
...
#ifdef CONFIG_USB_ANDROID_QCT_DIAG
			diag_smd_enable(ctxt->ch, "btdiag_ioctl", SMD_FUNC_CLOSE);
#else
			smd_close(ctxt->ch);
#endif
