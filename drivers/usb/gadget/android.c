...
static void release_functions(struct android_dev *dev)
{
	...
	list_for_each_entry(f, &android_config_driver.functions, list) {
		if (!f)
			continue;
		if (f && !f->hidden && f->release && f->dynamic) {
			printk(KERN_INFO "[USB-EP] %s release\n", f->name);
...
static void rebind_functions(struct android_dev *dev)
{
	...
	list_for_each_entry(f, &android_config_driver.functions, list) {
		if (f && !f->hidden && f->bind && f->dynamic) {
...
#ifdef CONFIG_USB_GADGET_DYNAMIC_ENDPOINT
	composite_mute_disconnect(dev->cdev->gadget);
	...
	list_for_each_entry(f, &android_config_driver.functions, list) {
		...
		if (!strcmp(f->name, "usb_mass_storage")) {
			...
			if (f->hidden) {
				...
				/* RndisDisableMPDecision: not lock cpu frequency and disable cpu1 for holiday*/
				if (!is_perf_lock_active(&usb_rndis_perf_lock) && !dev->RndisDisableMPDecision)
					perf_lock(&usb_rndis_perf_lock);
				if (dev->enable_fast_charge)
					dev->enable_fast_charge(1);
			}
			f->hidden = 0;
...
static int android_probe(struct platform_device *pdev)
{
	...
	if (pdata) {
		...
		dev->enable_fast_charge = pdata->enable_fast_charge;
		dev->RndisDisableMPDecision = pdata->RndisDisableMPDecision;
